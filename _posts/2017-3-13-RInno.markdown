...
tags: R
---
RStudio publishes great articles about deploying shiny apps on their website; the [local deployment][local-deployment] article is one of my favorites. You will find instructions about how to use shinyapps.io, Amazon Web Services, [Digital Ocean][do-referral], Microsoft Azure, or now RStudio Connect to deploy your app, but clients/organizations often request that your shiny app be hosted on-premise *without installing any new software* (RStudio Connect/Shiny Server). They also want the apps to be available to non-technical staff who may or may not have R installed. Each manual, r-specific step required to deploy your shiny app is a barrier to entry that we want to avoid.
...
- Can be easily uninstalled using Windows Control Panel
- Low maintenance for R developers because most steps are fully automated
- Many users don't even know R is being used
...
{% highlight r %}
example_app(app_dir = "app")  # This function has a bug on CRAN, so make sure to install from github
{% endhighlight %}
...
You should customize the messaging on the first page of your installer by updating the text in *infobefore.txt*, and replace *default.ico* with an icon that helps people deduce what your app does. Then call `compile_iss`.
