...
Because R/RStudio have often been approved for desktop computers, RInno helps push back the need for a software/cloud host security assessment by allowing us to deploy local shiny apps using approved, local copies of R. After organizations start using locally deployed shiny apps, it is our hope that they will build demand for hosted solutions without requiring as much maintenance as a fully manual local deployment.
