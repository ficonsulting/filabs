...
Because R/RStudio have often been approved for desktop computers, RInno solves this problem by allowing us to deploy local shiny apps that can run on a local copy of R. After organizations start using locally deployed shiny apps, it is our hope that they will build demand for hosted solutions without requiring as much maintenance as a fully manual local deployment.
